...
int dpll_cascading_blocker_hold(struct device *dev)
{
	...
	if (list_was_empty && in_dpll_cascading) {
		...
		printk(KERN_ERR "omap4_dpll_low_power_cascade_exit\n");
	}
...
int dpll_cascading_blocker_release(struct device *dev)
{
	...
	if (list_empty(&dpll_cascading_blocker_list) && in_dpll_cascading )
		goto out;
	...
	if (list_empty(&dpll_cascading_blocker_list)
		...
		printk(KERN_ERR "omap4_dpll_low_power_cascade_enter try!\n");			
		ret = omap4_dpll_low_power_cascade_enter();

		if( ret ){
			printk(KERN_ERR "omap4_dpll_low_power_cascade_enter failed!\n");			
			in_dpll_cascading = false;
		}else{
			printk(KERN_ERR "omap4_dpll_low_power_cascade_enter success!\n");			
		}
	}
...
int omap4_dpll_low_power_cascade_enter()
{
	...
	omap_smartreflex_disable_and_update_volt(vdd_mpu);
	omap_smartreflex_disable_and_update_volt(vdd_iva);
	omap_smartreflex_disable_and_update_volt(vdd_core);
	...
	if (dev) {
		opp = opp_find_freq_exact(dev, LP_196M_RATE, false);
		opp_enable(opp);
	}
	...
	if (dev) {
		opp = opp_find_freq_exact(dev, LP_98M_RATE, false);
		opp_enable(opp);
	}
	...
	if (ret) {
		pr_err("%s: failed reparenting DPLL_PER bypass clock to CLKINPULOW\n",
				__func__);
...
//	__raw_writel(0, OMAP4430_CM_L4_WKUP_CLKSEL);
	...
	if (dev) {
		opp = opp_find_freq_exact(dev, LP_196M_RATE, true);
		opp_disable(opp);
	}
	...
	if (dev) {
		opp = opp_find_freq_exact(dev, LP_98M_RATE, true);
		opp_disable(opp);
	}
