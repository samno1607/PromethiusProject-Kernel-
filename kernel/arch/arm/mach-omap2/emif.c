...
void cancel_out(u32 *num, u32 *den)
{
 ...
 */
void get_period(u32 freq_hz,  u32 den_limit, u32 *period_num, u32 *period_den)
{
...
static const struct lpddr2_timings *get_timings_table(
		const struct lpddr2_timings const *const *device_timings,
		u32 freq)
{
	u32 i, temp, freq_nearest;
	const struct lpddr2_timings *timings = 0;
 ...
 * on CS1
 */
static u32 get_sdram_config_reg(const struct lpddr2_device_info *
				 cs0_device,
				 const struct lpddr2_device_info *
				 cs1_device,
			      ...
			      bool volt_ramp)
{
	...
	mask_n_set(zq, OMAP44XX_REG_ZQ_ZQINIT_MULT_SHIFT,
			OMAP44XX_REG_ZQ_ZQINIT_MULT_MASK,
			REG_ZQ_ZQINIT_MULT - 1);
	...
	mask_n_set(zq, OMAP44XX_REG_ZQ_DUALCALEN_SHIFT,
			OMAP44XX_REG_ZQ_DUALCALEN_MASK,
			REG_ZQ_DUALCALEN_DISABLE);
 ...
 */
static void setup_registers(u32 emif_nr, struct emif_regs *regs,
			u32 volt_state)
{
	u32 temp, zq, read_idle;
	void __iomem *base = emif[emif_nr].base;

	__raw_writel(regs->ref_ctrl,
...
static struct emif_regs *get_regs(u32 emif_nr, u32 freq)
{
	 ...
	 */
	if ((emif_nr == EMIF1) ||
	   ((emif_nr == EMIF2) && (emif_devices[EMIF1] == emif_devices[EMIF2])))
...
static int do_emif_setup_registers(u32 emif_nr,
				   u32 freq,
				   u32 volt_state)
{
	struct emif_regs *regs;
...
/*
 * omap_emif_setup_volt_sensitive_registers - setup the voltage sensitive
