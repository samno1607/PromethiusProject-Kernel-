        ...
        if (!enabled) return;
          if (!suspend) {
                ...
                for_each_cpu(cpu, &tmp_mask) {
                  ...
                  smp_rmb();
                  __cpufreq_driver_target(pcpu->cur_policy, 1008000, CPUFREQ_RELATION_L); //this value should NEVER go under 1008000
                }
                ...
                for_each_cpu(cpu, &tmp_mask) {
                  pcpu = &per_cpu(cs_cpu_dbs_info, cpu);
                  smp_rmb();
                  __cpufreq_driver_target(pcpu->cur_policy, 600000, CPUFREQ_RELATION_H); //this value should NEVER go under 600000
                }
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (max_load < 85 && this_dbs_info->requested_freq == 1200000)	
		__cpufreq_driver_target(policy, 1008000,
				CPUFREQ_RELATION_H);
}
